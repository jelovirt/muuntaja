...
logger.info("dita.map.eclipse.content:")
...
logger.info("dita.map.eclipsecontent.init:")
if (!$.contains("args.eclipsecontent.toc")) {
$("args.eclipsecontent.toc") = $("dita.map.filename.root")}
$("content.link.ext") = ".html?srcext=dita"
}

/** Build EclipseContent TOC file */
def ditaMapEclipsecontentToc() {
logger.info("dita.map.eclipsecontent.toc:")
...
logger.info("dita.map.eclipsecontent.index:")
depends(("dita.map.eclipsecontent.init", ditaMapEclipsecontentInit))
if (job.getFileInfo.find(_.format == "ditamap").isEmpty) {
return}
...
logger.info("dita.map.eclipsecontent.plugin:")
depends(("dita.map.eclipsecontent.init", ditaMapEclipsecontentInit))
...
logger.info("dita.topics.eclipse.content:")
