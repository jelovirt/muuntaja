...
logger.info("set_odt_output_tempdir:")
...
logger.info("clean_output_tempdir:")
if ($.contains("$flag")) {
return}
...
logger.info("odt.image-metadata:")
...
logger.info("dita.topic.odt.stylesfile:")
if (job.getFileInfo.find(_.format == "ditamap").isDefined) {
return}
...
logger.info("dita.map.odt.stylesfile:")
if (job.getFileInfo.find(_.format == "ditamap").isEmpty) {
return}
...
logger.info("dita.out.odt.manifest.file:")
...
logger.info("dita.odt.package.topic:")
...
logger.info("dita.odt.package.map:")
